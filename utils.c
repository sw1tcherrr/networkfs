...
char* escape_url(char const* pref, char const* str, size_t len) {
	int i = 0;
	char sym[3];
	char* escaped = kmalloc(3 * len + strlen(pref), GFP_KERNEL);
	ERR_WRAPPER(!escaped, "escape_url: memory error", exit)

	strcat(escaped, pref);
	for (i = 0; i < len; ++i) {
		sprintf(sym, "%%%02x", (int)str[i]);
		strcat(escaped, sym);
	}
...
int connect_to_server(const char *command, int params_count, const char *params[], const char *token, char *output_buf) {
	...
	error = *(int *)(recv_buf);
	recv_buf += 8;
	memcpy(output_buf, recv_buf, message_len - 8);
